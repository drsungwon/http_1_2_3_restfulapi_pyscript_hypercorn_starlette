...
def get_http_url():
    if document.querySelector("#HTTP-1").checked == True:
        SERVER_ADDRESS = 'http://127.0.0.1:8000/membership_api/'
    elif document.querySelector("#HTTP-2").checked == True:
        SERVER_ADDRESS = 'https://127.0.0.1:8000/membership_api/'
    else:
...
async def do_create_by_post(evt): 
    ...
    display_msg(["Sends HTTP POST {} with key:value and shows results.".format(get_http_url()),
...
async def do_read_by_get(evt): 
    ...
    display_msg(["Sends HTTP GET {} with key and shows results.".format(get_http_url()),
...
async def do_update_by_put(evt): 
    ...
    display_msg(["Sends HTTP PUT {} with key:value and shows results.".format(get_http_url()),
...
async def do_delete_by_delete(evt): 
    ...
    display_msg(["Sends HTTP DELETE {} with key and shows results.".format(get_http_url()),
